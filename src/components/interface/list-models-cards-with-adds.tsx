...
export const ListModelsCardsWithAdds = ({ className, cardsPerAdd = 6, query, models, ...props }: ListModelsCardsWithAddsProps) => {
    ...
    {models.map((model, index) => {
      ...
      return (
        <React.Fragment key={model.id}>
          ...
          {index % cardsPerAdd ===  cardsPerAdd - 1 && index !== models.length - 1 && (
           <div
            key={`ad-${index}`}
            ...
            <Image
              ...
              fill
              src={addsImg[0].profileImage?.url ?? "/default-profile.jpg"}
              alt={model.username}
            />
